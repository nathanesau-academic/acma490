...
This gives an amount of $\Sexpr{round(amount, 0)}$.
...
1 & 1 & \Sexpr{round(first[1], 5)} & 0 & 0 \\
1 & 10 & \Sexpr{round(first[2], 5)} & \Sexpr{round(sdev[2], 5)} & \Sexpr{round(skew[2], 5)} \\
10 & 1 & \Sexpr{round(first[3], 5)} & 0 & 0 \\
10 & 10 & \Sexpr{round(first[4], 5)} & \Sexpr{round(sdev[4], 5)} & \Sexpr{round(skew[4], 5)} \\ \hline
\end{tabular}
\end{center}
\end{table}
...
This gives $Var[Z] = \Sexpr{round(VarZ, 8)}$.

\begin{enumerate}[(i)]
\item The variance of the random discounted values is $\Sexpr{round(InvestmentRisk, 8)}$.
\item The variance of the present value of the benefit is $\Sexpr{round(InsuranceRisk, 8)}$.
\end{enumerate}
...
\begin{enumerate}[(i)]
\item The insurance risk is $\Sexpr{round(InsuranceRisk, 8)}$.
\item The investment risk is $\Sexpr{round(InvestmentRisk, 8)}$.
\end{enumerate}
...
This gives $sd[Z(c)/c] = \Sexpr{round(sd10, 8)}$ for $c = 10$ and $sd[Z(c)/c] = \Sexpr{round(sdInf, 8)}$ for $c = \infty$.

\item The limiting variance is $\Sexpr{round(sdInf^2, 8)}$. This is the same as component (1) in exercise 3.1.
